...
size_t OrderedCount(const TCGraph &g) {
  size_t total = 0;
...
bool WorthRelabelling(const TCGraph &g) {
  int64_t average_degree = g.num_edges() / g.num_nodes();
  if (average_degree < 10)
    return false;
  SourcePicker<TCGraph> sp(g);
