...
int64_t BUStep(CSR &g, pvector<int32_t> &parent, Bitmap &front,
			   Bitmap &next) {
...
int64_t TDStep(CSR &g, pvector<int32_t> &parent,
...
void BitmapToQueue(CSR &g, const Bitmap &bm,
				   SlidingQueue<int32_t> &queue) {
	#pragma omp parallel
	{
		QueueBuffer<int32_t> lqueue(queue);
...
pvector<int32_t> InitParent(CSR &g) {
...
pvector<int32_t> DOBFS(CSR &g, int32_t source, int alpha = 15, int beta = 18) {
