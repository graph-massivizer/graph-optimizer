...
def symbolic_model(T_float_div, T_float_mult, T_float_sub, T_float_add, T_malloc, T_int_add, T_int_mult, T_int_gt, cache_linesizes, mem_access_times, int_size):
    ...
    T_main_loop = f'100*n*2*({T_neighbour_scan} + {T_float_add + T_float_mult + 2*T_mem_read + T_float_div} + 2*{T_mem_write}) + 2*m*({(T_float_add + T_mem_read)})'
    T_pagerank = f'({T_init} + {T_init_arrays} + {T_main_loop}) / 10000'
    return T_pagerank
