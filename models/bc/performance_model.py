...
symbolical_model_parameters = ["T_insert_max", "T_int_add", "T_malloc_n", "T_heap_extract_min", "T_heap_decrease_key", "cache_linesizes", "mem_access_times", "bool_size", "int_size", "float_size"]

def symbolic_model(T_insert_max, T_int_add, T_malloc_n, T_heap_extract_min, T_heap_decrease_key, cache_linesizes, mem_access_times, bool_size, int_size, float_size):
    ...
    T_bc = f'(n*{T_float_mem_write} + s*({T_dijkstra + T_vprop_mem_write} + {T_malloc_n}) + s*(s-1)*({T_inner_loop})) / 1000000'
...
def predict(hardware):
    ...
    return symbolic_model(microbenchmarks['T_insert_max'], microbenchmarks['T_int_add'], microbenchmarks['T_malloc_n'], microbenchmarks['T_heap_extract_min'], microbenchmarks['T_heap_decrease_key'],
