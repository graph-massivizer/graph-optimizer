...
template void pretty_print_array<int>(int *, int, std::string);

void pretty_print(std::vector<int> v) {
    printf("[");
    fflush(stdout);
...
void pretty_print(CArray<float> v) {
    printf("[");
    for (int i = 0; i < v.size - 1; i++) {
        printf("%f,", v.data[i]);
    }
...
template void read_vector_CArray<float>(CArray<float>*, char*);
template void read_vector_CArray<int>(CArray<int>*, char*);
...
// template <>
// void read_vector_CArray<int32_t>(CArray<int> *V, char* filename) {
//     GrB_Matrix M;
//     read_graph_GB(&M, filename);
...
template void write_vector_CArray<float>(CArray<float>, char*);
template void write_vector_CArray<int>(CArray<int>, char*);
